...
namespace LeaveManagement.Web.Controllers
{
    public class EmployeesController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            var model=mapper.Map<List<EmployeeListVM>>(users);
            return View(model);
        }
        ...
        public async Task<ActionResult> EditAllocation(int id, LeaveAllocationEditVM model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    return RedirectToAction(nameof(ViewAllocations), new {id=model.Id});
            ...
            model.LeaveType = mapper.Map<LeaveTypesVM>(await leaveTypeRepository.GetAsync(model.LeaveTypeId));
            return View(model);
        }
